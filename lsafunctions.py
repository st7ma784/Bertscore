...
def no_for_loop_v2_triu_MyLinearSumAssignment(rewards:torch.Tensor,maximize=True,tril=False):
    ...
    cost_neg,next_highest_fn,final_fn=((1e9,torch.min,torch.argmin),(0,torch.max,torch.argmax))[maximize]
...
def recursiveLinearSumAssignment(rewards:torch.Tensor,maximize=True,factor=0.8):
    cost_neg,next_highest_fn,final_fn=((torch.tensor(float('inf')),torch.min,torch.argmin),(torch.tensor(float('-inf')),torch.max,torch.argmax))[maximize] 
...
def recursiveLinearSumAssignment_grad(rewards:torch.Tensor,maximize=True,factor=0.8):
    cost_neg,next_highest_fn=((torch.tensor(float('inf')),torch.min),(torch.tensor(float('-inf')),torch.max))[maximize] 
...
def recursiveLinearSumAssignment_v2(rewards:torch.Tensor,maximize=True,factor=1):
    final_fn=((torch.argmin),(torch.argmax))[maximize] 
